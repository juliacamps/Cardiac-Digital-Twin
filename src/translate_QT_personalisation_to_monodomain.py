...
if __name__ == '__main__':
    ...
    # for_monodomain_figure_result_file_name = visualisation_dir + anatomy_subject_name + '_' + source_resolution + '_' + result_tag + '_population.png'
    # Translation to Monodomain
    for_monodomain_dir = results_dir + 'for_monodomain_population/'
    if not os.path.exists(for_monodomain_dir):
        os.mkdir(for_monodomain_dir)
    for_monodomain_biomarker_result_file_name_start = for_monodomain_dir + anatomy_subject_name + '_' + target_resolution + '_nodefield_' + result_tag + '-biomarker_'
    for_monodomain_biomarker_result_file_name_end = '.csv'
    for_monodomain_parameter_population_file_name = for_monodomain_dir + anatomy_subject_name + '_' + source_resolution + '_' + result_tag + '_drug_parameter_population.csv'
    for_monodomain_figure_result_file_name = for_monodomain_dir + anatomy_subject_name + '_' + source_resolution + '_' + result_tag + '_population.png'
    # # Best discrepancy
    ...
    v3_name = 'V3' # hyperparameter_dict['v3_name'] # TODO
    v5_name = 'V5' # hyperparameter_dict['v5_name'] # TODO
    ...
    if True:
        ...
        ####################################################################################################################
        # Step 17: Select population and save their biomarkers for translation to monodomain simulations.
        # TODO only for Maxx Holmes 2024
        # TODO only for Maxx Holmes 2024
        # TODO only for Maxx Holmes 2024
        print('Step 17: Select particle with best T wave match in final population and save their biomarkers.')
        ...
        discrepancy_metric_pcc = DiscrepancyECG(
            error_method_name=error_method_name_pcc)  # TODO only for Maxx Holmes 2024
        discrepancy_population_maxx = discrepancy_metric_pcc.evaluate_metric_population(
            # TODO only for Maxx Holmes 2024
            predicted_data_population=population_ecg[:, 2:, 100:],
            target_data=clinical_ecg[2:, 100:])  # TODO only for Maxx Holmes 2024
        selected_i = np.argmax(discrepancy_population_maxx)  # TODO only for Maxx Holmes 2024
        uncertainty_parameter_population = parameter_population[selected_i, :]
        uncertainty_parameter_population = uncertainty_parameter_population[np.newaxis, :]
        uncertainty_population_lat, uncertainty_population_vm = evaluator_ep.simulate_parameter_population(
            parameter_population=uncertainty_parameter_population)  # TODO only for Maxx Holmes 2024
        # print('selected_population_vm ', selected_population_vm.shape)  # TODO only for Maxx Holmes 2024
        # node_repol = generate_repolarisation_map(vm=selected_population_vm)  # TODO only for Maxx Holmes 2024
        ...
        selected_population_ecg = evaluator_ecg.simulate_parameter_population(
        ...
        for uncertainty_i in range(uncertainty_parameter_population.shape[0]):
            uncertainty_biomarker_result_file_name = for_monodomain_biomarker_result_file_name_start + str(
                uncertainty_i) + for_monodomain_biomarker_result_file_name_end
    ...
    axes, fig = visualise_ecg(ecg_list=population_ecg, lead_name_list=lead_names, axes=axes,
                              ...
                              linewidth=0.5)
    # Selected particle from the inferred population
    axes, fig = visualise_ecg(ecg_list=selected_population_ecg, lead_name_list=lead_names, axes=axes,
                              ecg_color='r', fig=fig, label_list=None,
                              linewidth=1.)
