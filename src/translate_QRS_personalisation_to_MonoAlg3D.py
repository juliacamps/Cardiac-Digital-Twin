...
if __name__ == '__main__':
    ...
    from ecg_functions import PseudoEcgTetFromVM, PseudoQRSTetFromStepFunction
    ...
    from cellular_models import CellularModelBiomarkerDictionary, MitchellSchaefferAPDdictionary, StepFunctionUpstrokeEP
    from electrophysiology_functions import ElectrophysiologyAPDmap, ElectrophysiologyUpstrokeStepFunction
    ...
    # figure_result_file_name = visualisation_dir + anatomy_subject_name + '_' + inference_resolution + '_' + result_tag + '_population.png'
    # # Best discrepancy
    # translation_dir = results_dir + 'best_discrepancy/'
    # if not os.path.exists(translation_dir):
    #     os.mkdir(translation_dir)
    # lat_result_file_name = translation_dir + anatomy_subject_name + '_' + monodomain_resolution + '_nodefield_' + result_tag + '-lat.csv'
    # vm_result_file_name = translation_dir + anatomy_subject_name + '_' + monodomain_resolution + '_nodefield_' + result_tag + '-vm.csv'
    # best_parameter_result_file_name = translation_dir + anatomy_subject_name + '_' + inference_resolution + '_' + result_tag + '-best-parameter.csv'
    ...
    # for_monodomain_best_biomarker_result_file_name = for_monodomain_dir + anatomy_subject_name + '_' + monodomain_resolution + '_nodefield_' + result_tag + '-biomarker_best.csv'
    # for_monodomain_best_Purkinje_result_file_name = for_monodomain_dir + anatomy_subject_name + '_' + monodomain_resolution + '_nodefield_' + result_tag + '-Purkinje.csv'
    # for_monodomain_root_node_result_file_name = for_monodomain_dir + anatomy_subject_name + '_' + monodomain_resolution + '_nodefield_' + result_tag + '-Purkinje_root_node.csv'
    ...
    # transmural_speed = hyperparameter_dict[transmural_speed_name]
    ...
    #     fibre_speed=fibre_speed, sheet_speed=transmural_speed, normal_speed=normal_speed,
    ...
    ecg_model = PseudoQRSTetFromStepFunction(electrode_positions=geometry.get_electrode_xyz(), filtering=filtering,
                                             ...
                                             nodes_xyz=geometry.get_node_xyz(),
                                             normalise=normalise, reference_ecg=clinical_ecg_raw,
    ...
    # discrepancy_metric = DiscrepancyECG(
    #     error_method_name=error_method_name)  # TODO: add weighting control between PCC and RMSE
    # # Clear Arguments to prevent Argument recycling
    ...
    print('Step 12: Evaluate their ECG.')
    ...
    uncertainty_proportion = 0.0   # 0% of the population size
    ...
    print('16: Iterate for all particles chosen to represent the uncertainty of the inference.')
    ...
    for uncertainty_i in range(uncertainty_parameter_population.shape[0]):
        ...
        unprocessed_node_biomarker = pd.DataFrame()
        # LAT AND REPOL MAPS
        unprocessed_node_biomarker[activation_time_map_biomarker_name] = uncertainty_population_lat[uncertainty_i, :]
        unprocessed_node_biomarker = initialise_pandas_dataframe(df=unprocessed_node_biomarker)
    ...
    print('Step 18: Visualise ECGs and their metrics for the final population.')
