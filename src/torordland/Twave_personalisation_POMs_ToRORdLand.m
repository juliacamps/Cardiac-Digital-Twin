function [] = Twave_personalisation_POMs_ToRORdLand(nb_models,celltype)
...
sigma1 = 1.94690948;
sigma2 = 1.74285262;
Istim = -sf* (A1 * exp(-(t-mu1).^2/(2*sigma1^2)) - A2 * exp(-(t-mu2).^2/(2*sigma2^2)));
...
%     celltype = 'endo';
    ...
    [time, X] = modelRunner(X0, options, params(i), beats, ignoreFirst);
    ...
    endo_apd40(i) = DataReporter.getAPD(currents.time, currents.V, 0.4);
    endo_apd50(i) = DataReporter.getAPD(currents.time, currents.V, 0.5);
    endo_apd60(i) = DataReporter.getAPD(currents.time, currents.V, 0.6);
    endo_apd70(i) = DataReporter.getAPD(currents.time, currents.V, 0.7);
    endo_apd80(i) = DataReporter.getAPD(currents.time, currents.V, 0.8);
    endo_apd90(i) = DataReporter.getAPD(currents.time, currents.V, 0.9);
    endo_dvdt_max(i) = DataReporter.getPeakDVDT(currents.time, currents.V, -1.0);
    endo_CTD50(i) = DataReporter.getAPD(currents.time, currents.Cai, 0.5);
    endo_CTD90(i) = DataReporter.getAPD(currents.time, currents.Cai, 0.9);
...
writetable(T,['/data/Personalisation_projects/meta_data/cellular_data/biomarkers_table_' celltype '.csv'],'WriteRowNames',false)

writematrix(v, ['/data/Personalisation_projects/meta_data/cellular_data/torordland_calibrated_pom_aps_1000Hz_' celltype '.csv']);
