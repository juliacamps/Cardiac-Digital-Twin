        function [] = Twave_personalisation_POMs_ToRORd_IKs_IKr_not_converged(nb_models, celltype, stimAmp, stimulus_function_str, result_dir)
    ...
    %% Debug for Alya
    param_baseline.IKr_Multiplier = 0.7; % 1.0;
    param_baseline.IKs_Multiplier = 0.02; % 0.02;
    [time, X_converged_baseline] = modelRunner_ToRORd(X0, options, param_baseline, beats, ignoreFirst);
    ...
    [value, index] = min(abs(currents.V + 80));
    currents.time(index)
    ...
    if strcmp(stimulus_function_str, 'step')
        param_dictionary.model = @model_ToRORd_step_stimulus_rescaled_IKs; % which model is to be used
    elseif strcmp(stimulus_function_str, 'diffusion')
        param_dictionary.model = @model_ToRORd_diffusion_current_rescaled_IKs; % which model is to be used
    end
    param_dictionary.verbose = false; % printing numbers of beats simulated.
    param_dictionary.celltype = celltype;

    % What if instead we keep the correlation between GKs and GKr?
    % Before:
%     lhs_multipliers = lhsdesign(nb_models, 2);
%     figure;
%     histogram(lhs_multipliers(:, 1));
%     title('1');
%     figure;
%     histogram(lhs_multipliers(:, 2));
%     title('2');
    % New:
    gks_linspace = linspace(0., 1., floor(2*nb_models/3))' * 49.98 + 0.02;
    gkr_linspace = linspace(0., 1., ceil(nb_models/3))'*0.3 + 0.7;
    gks_full = [zeros([ceil(nb_models/3) 1]) + gks_linspace(1); gks_linspace];
    gkr_full = [gkr_linspace ; zeros([floor(2*  nb_models/3) 1]) + gkr_linspace(end)];
    
%     lhs_multipliers = [lhs_multiplier, lhs_multiplier];
    lhs_multipliers = [gks_full, gkr_full];
...
%     lhs_multipliers(:,2) = lhs_multipliers(:,2) * 4.8 + 0.2;
...
%         params_dictionary(ipopulation).Ito_Multiplier = 1.0 ; %lhs_multipliers(ipopulation,1);
        ...
        params_dictionary(ipopulation).IKr_Multiplier = lhs_multipliers(ipopulation,2);
    end
...
%     activaiton_time_shift = activity_cut_index;
    ...
    sf_IKr = lhs_multipliers(calibrated_population_biomarkers_index,2);
    ...
    biomarkers = table(activation_time, apd40, apd50, apd60, apd70, apd80, apd90, tri_90_40, dvdt_max, ...
        CTD50, CTD90, vpeak, RMP, CaT_max, CaT_min, sf_IKs, sf_IKr);
    ...
    disp(size(calibrated_time));
    figure;
    title(['Calibrated in blue with uncalibrated in black, celltype: ' celltype])
    hold on;
...
%     end
    disp('Time per model: ')
    disp(toc/nb_models)
    for cellmodel_i = 1:size(shared_vm, 2)
        plot(shared_time(cellmodel_i).value, shared_vm(cellmodel_i).value , 'k');
    end
    for cellmodel_i = 1:size(calibrated_vm, 1)
    ...
    title('Activation times histogram');
    xlabel('(ms)')
    ...
    t = tiledlayout(1,1);
    ax1 = axes(t);
    plot(ax1, sf_IKs, apd90, '.r');
    ax1.XColor='r';
    ax1.YColor='r';
    ax2 = axes(t);
    plot(ax2, sf_IKr, apd90, '.k');
    ...
    xlabel(ax1, 'sf IKs');
    xlabel(ax2, 'sf IKr');
    title(['Sf_IKs and Sf_IKr vs APD90 ', celltype]);
end
