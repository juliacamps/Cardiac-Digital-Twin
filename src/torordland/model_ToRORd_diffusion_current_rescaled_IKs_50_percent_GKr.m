function output=model_ToRORd_diffusion_current_rescaled_IKs_60_percent_GKr(t,X,flag_ode, cellType, ICaL_Multiplier, ...
...
%   nperm=2.2;
%   ca50=2.5 * ca50_Multiplier;
%   Tref=40.5;
%   nu=1;
%   mu=1;
% else
%   nperm=2.036;
%   ca50=0.805 * ca50_Multiplier;
%   %ca50 = 0.5;
%   %ca50 = 0.7;
%   Tref=120;
%   nu=7;
%   mu=3;
% end
% k_ws=0.004*kws_Multiplier;
% k_uw=0.026*kuw_Multiplier;
%
% lambda_min=0.87;
% lambda_max=1.2;
%
...
%% IKCa
IKCa = getIKCa(v, cai, cass, EK, IKCa_Multiplier);
...
%% Jrel
[Jrel, dJrel_np, dJrel_p] = getJrel_ORd2011(Jrel_np, Jrel_p, ICaL_ss,cass, cajsr, fJrelp, celltype, Jrel_Multiplier, taurelp_Multiplier);
...
%     Istim=0.0;
% end
Istim = get_diffusion_current(t, Istim_sf);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%update the membrane voltage

dv=-(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+IKCa+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab+ + I_ClCa+I_Clbk + Istim);
...
tjca = 60; %75; IKs rescaling to allow greater APD gradient (also requires IKs and IKr scaling)
...
% IKs rescaling to allow greater APD gradient (also requires GKs and tjca
% scaling)
GKr = 0.0321 * sqrt(ko/5) * IKr_Multiplier * 0.5; % 1st element compensates for change to ko (sqrt(5/5.4)* 0.0362)
if celltype==1
    GKr=GKr*1.3;
elseif celltype==2
    GKr=GKr*0.8;
...
GKs= 0.0011 * IKs_Multiplier * 5.0; % IKs rescaling to allow wider APD spatial gradient (also need IKr and tjca scaling)
if celltype==1
    GKs=GKs*1.4;
end
...
function [IKCa] = getIKCa(v, cai, cass, EK, IKCa_Multiplier)
fIKCass = 0.8;
kdikca = 6.05e-4;
ikcan = 3.5;
GKCa = 0.003 * IKCa_Multiplier;
IKCa_ss = GKCa * fIKCass * cass^ikcan / (cass^ikcan + kdikca^ikcan) * (v-EK);
IKCa_i = GKCa * (1.0-fIKCass) * cai^ikcan / (cai^ikcan + kdikca^ikcan) * (v-EK);
IKCa = IKCa_ss + IKCa_i;
end
...
%% Jrel
function [Jrel, dJrelnp, dJrelp] = getJrel_ORd2011(Jrelnp, Jrelp, ICaL, cass, cajsr, fJrelp, celltype, Jrel_Multiplier, taurelp_Multiplier)
jsrMidpoint = 1.7;
